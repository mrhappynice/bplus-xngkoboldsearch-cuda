...
MODEL_PATH="/usr/local/bin/models/$(basename "$MODEL_URL")"
...
# curl -o /usr/local/bin/$(basename "$MODEL_URL") "$MODEL_URL" &

chmod +x /usr/local/bin/koboldcpp-linux-x64 &
  
./usr/local/bin/koboldcpp-linux-x64 \
  --model "$MODEL_PATH" \
  --usecuda \
  --port 1234 \
  --contextsize 8000 \
